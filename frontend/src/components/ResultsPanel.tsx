    ...
    <div className="space-y-7">
      <section className="overflow-hidden rounded-3xl border border-forest/10 bg-paper shadow-[0_18px_60px_rgb(9_29_24/8%)]" aria-labelledby="result-title">
        <div className="relative overflow-hidden bg-ink px-6 py-7 text-paper sm:px-8 sm:py-9">
          <div className="absolute -top-20 -right-10 h-56 w-56 rounded-full border-[26px] border-sarawak-yellow/15" />
          <div className="relative flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-4 max-w-xl text-sm leading-6 text-paper/80">Use this as a field cue, then compare the audio, habitat and candidate species before recording an observation.</p>
            </div>
        ...
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.08fr_.92fr]">
          <div className="space-y-5">
            {quality && (
              <div className={`rounded-2xl border p-4 ${qualityStyle}`}>
                <div className="flex gap-3">
                  <Icon name={result.needs_review ? 'alert' : 'check'} className="mt-0.5 h-5 w-5 shrink-0" />
                  <div>
                    <p className="text-sm font-semibold">{quality.label}</p>
                    <p className="mt-1 text-sm leading-5 opacity-85">{quality.message}</p>
                  </div>
                </div>
              </div>
            )}
            <div className="rounded-2xl border border-forest/10 bg-canvas p-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-semibold text-ink">Sound evidence</h3>
                <span className="text-xs font-medium text-muted">Mel spectrogram</span>
              </div>
              <img src={result.spectrogram_url} alt={`Mel-spectrogram of the uploaded recording, with ${formatSpeciesName(result.species)} as the leading candidate`} className="mt-3 w-full rounded-xl border border-forest/10 bg-paper" />
              <audio controls src={result.audio_url} className="mt-3 h-10 w-full accent-sarawak-red">
                <track kind="captions" />
              </audio>
            </div>
          ...
          <div className="space-y-5">
            ...
            <FieldActions result={result} profile={profile} species={species} />
          </div>
        </div>
      </section>

      {profile ? (
        <SpeciesProfileCard profile={profile} />
      ) : (
        <div className="rounded-2xl border border-forest/10 bg-paper p-5 text-sm text-muted">Loading the Sarawak field guide…</div>
      )}

      <button type="button" onClick={onReset} className="inline-flex items-center gap-2 rounded-xl border border-forest/20 bg-paper px-4 py-2.5 text-sm font-semibold text-forest transition hover:border-sarawak-red/40 hover:bg-leaf/50">
        Analyse another recording <Icon name="arrow-right" className="h-4 w-4" />
      </button>
    </div>
  )
}
