  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight text-ink sm:text-5xl">Follow the sound, then verify the story.</h1>
        <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-forest/75 sm:text-base">BirdSense compares your audio with its current Bornean bird classifier and presents the evidence needed for a more careful field decision.</p>
      </div>

      <div className="mx-auto mt-10 max-w-5xl">
        {status !== 'success' && <UploadDropzone onFileSelected={handleFileSelected} disabled={status === 'loading'} />}

        {status === 'loading' && (
          <div className="mt-5 flex items-center justify-center gap-3 rounded-2xl border border-sarawak-yellow/40 bg-sarawak-yellow/15 px-4 py-4 text-sm font-medium text-forest" role="status">
            <span className="relative flex h-4 w-4"><span className="pulse-ring absolute inline-flex h-full w-full rounded-full bg-sarawak-red" /><span className="relative inline-flex h-4 w-4 rounded-full bg-sarawak-red" /></span>
            Extracting sound features and comparing candidates…
          </div>
        )}
        ...
        <section className="mx-auto mt-14 max-w-5xl" aria-labelledby="recording-tips-title">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {RECORDING_TIPS.map(([title, body], index) => (
              <div key={title} className="rounded-2xl border border-forest/10 bg-paper p-5">
