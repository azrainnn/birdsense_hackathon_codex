    ...
    return jsonify(
        {
            ...
            "spectrogram_duration_seconds": round(float(spectrogram_info["duration_seconds"]), 2),
            "spectrogram_plot_bounds": {
                key: round(float(value), 3) for key, value in spectrogram_info["plot_bounds"].items()
            },
            ...
            "window_predictions": [
                {
                    "start_time": round(float(window["start_time"]), 2),
                    "end_time": round(float(window["end_time"]), 2),
                    "confidence": round(float(window["confidence"]), 4),
                }
                for window in result.get("window_predictions", [])
            ],
            "timeline": timeline,
        }
    )
